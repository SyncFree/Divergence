...
public ConfigContainer(Properties config, boolean check)
{
	...
	for (i = 0; i < prots.length; ++i) {
		//System.err.println(" ===== W ===== Storing general protocol: '" + prots[i] + "' with id: " + (i));
		protocols.put(prots[i].substring(Configuration.PAR_PROT.length() + 1), Integer.valueOf(i));
	}
	int j = 0;
	prots = getNames(Configuration.PAR_SERVER_PROT); // read server-only protocols
	for (j = 0; j < prots.length; ++j) {
		//System.err.println(" ===== W ===== Storing Server only protocol: '" + prots[j] + "' with id: " + (i+j));
		protocols.put(prots[j].substring(Configuration.PAR_SERVER_PROT.length() +1), Integer.valueOf(i+j));
	}
	prots = getNames(Configuration.PAR_CLIENT_PROT); // read client-only protocols
	for (int k = 0; k < prots.length; ++k) {
		//System.err.println(" ===== W ===== Storing Client only protocol: '" + prots[k] + "' with id: " + (i+k));
		protocols.put(prots[k].substring(Configuration.PAR_CLIENT_PROT.length() +1), Integer.valueOf(i+k));
	}
...
{
	System.err.println("Fetching config value: " + name);
...
public int lookupPid(String protname)
{
	...
	System.err.println("Looking up pid of '" + protname + "' ======> " + ret);
	if (ret == null) {
		...
		for (; i < sret.length; i++) {
			...
			for (; j < names.length; ++j) {
				if (names[j].equals(type + "." + sret[i]))
					break;
				//System.err.println("value of names[" + j + "] = " + names[j] + " :: " + "value of sret[" + i + "] = " + sret[i]);
				if (type.equals(Configuration.PAR_PROT) && (names[j].equals(Configuration.PAR_SERVER_PROT + "." + sret[i]) || names[j].equals(Configuration.PAR_CLIENT_PROT + "." + sret[i])))
					break;
			}
			if (j == names.length) {
