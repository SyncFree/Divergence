...
public GeneralNode(String prefix) {
	...
	System.err.println("Instantiating GenralNode with prefix = " + prefix + " ==> Loading protocols");
	int i=0;
	...
	System.err.println("Instantiating GenralNode with prefix = " + prefix + " ==> Loading secondary protocols");
	for (int j = 0; j < names_specs.length; j++,i++) {
		CommonState.setPid(i);
		Protocol p = (Protocol)
				Configuration.getInstance(names_specs[j]);
		if(p == null) System.err.println(" ############### Failed to instantiate: " + names_specs[j]);
		protocol[i] = p;
	}
}
