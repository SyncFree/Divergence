 ...
 *
 * @author João Leitão (based on the code of Alberto Montressor)
 * @version $Revision: 1.5 $
 */
public class DCControlEvent
{
 ...
 * simulation.
 */
public DCControlEvent(Control control, Scheduler scheduler, int order)
{
	this.control = control;
	this.order = order;
	this.scheduler = scheduler;
	long next = scheduler.getNext();
	if( next>=0 ) DCDataStoreSimulator.addControlEvent(next, order, this);
}
