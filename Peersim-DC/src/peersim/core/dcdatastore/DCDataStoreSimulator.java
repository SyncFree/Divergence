...
* @see Configuration
 */
public class DCDataStoreSimulator
{
 ...
 * @config
 */
public static final String PAR_GRACE_PERIOD = "simulation.gracetime";	
 ...
 * 
 */
private static final String PAR_CLIENT_EVENT_GENERATOR = "simulation.clientmanager";
private static final String PAR_OPS_FILE = "simulation.clientopsfile";
//---------------------------------------------------------------------
//Fields
//---------------------------------------------------------------------
...
/** Time for running simulation after clients have no more ops */
private static long gracetime;
...
/** This class generates operations for clients to execute */
private static ClientOperationGenerator clientManager = null;
...
private static void scheduleControls()
{
	...
	for (int i=0; i < controls.length; i++) {
		new DCControlEvent(controls[i], ctrlSchedules[i], i);
	}
}
 ...
 *   typically the case.
 * @param dcControlEvent 
 *   The control event
 */
static void addControlEvent(long time, int order, DCControlEvent dcControlEvent)
{
...
// the caller, which must be from this package
	if(endtime == -1 || time < endtime )
		heap.add(time, dcControlEvent, null, (byte)0, order);
}
 ...
 * be used for dc datastore driven simulations. It checks for the existence of
 * config parameter {@value #PAR_GRACE_PERIOD}.
 */
public static final boolean isConfigurationDCDriven()
{
	return Configuration.contains(PAR_GRACE_PERIOD);
}
...
private static boolean executeNext() {
	...
	if( ev == null )
	{
		ClientOperationGenerationEvent event = clientManager.hasMoreOperations();
		if(event != null) {
			return loadClientOperations();
		} else {
			DCDataStoreSimulator.setEndtime();
		}
		
		System.err.println("DCDataStoreSimulator: queue is empty, quitting"+
		" at time "+DCCommonState.getTime());
		return true;
	}
	...
	if (time >= nextlog)
	{
		...
		//if( endtime != -1 && endtime-nextlog >= logtime ) nextlog+=logtime;
	...
	if (ev.node == null)
	{
		if(ev.event instanceof DCControlEvent) {
			return ((DCControlEvent) ev.event).execute();
		} else if (ev.event instanceof ClientOperationGenerationEvent) {
			System.err.println("Generating Client Operations.");
			return loadClientOperations();
		} else 
	...
	else if (ev.node.isUp() )
	{
		...
		else
		{
			...
			try {
				//System.err.println("Delivering '" + ev.event.getClass().getCanonicalName() + " to " + ev.node.getProtocol(pid).getClass().getCanonicalName());
...
private static boolean loadClientOperations() {
	//System.err.println("Loading next set of Client Operations");
...
public static void nextExperiment() 
{
	...
	if( Configuration.contains(PAR_PQ) ) 
		...
		heap = new Heap();
	gracetime = Configuration.getLong(PAR_GRACE_PERIOD);
	endtime = -1;
	logtime = Configuration.getLong(PAR_LOGTIME, Long.MAX_VALUE);

	// initialization
	System.err.println("DCDataStoreSimulator: resetting");
	CommonState.setPhase(DCCommonState.PHASE_UNKNOWN);
	DCCommonState.setPhase(DCCommonState.PHASE_UNKNOWN);
	CommonState.setTime(0);
	DCCommonState.setTime(0); // needed here
	controls = null;
	ctrlSchedules = null;
	nextlog = 0;
	GeoReplicatedDatastoreNetwork.reset();
	System.err.println("DCDataStoreSimulator: setting up client manager");
	try {
		clientManager = (ClientOperationGenerator) Class.forName(Configuration.getString(PAR_CLIENT_EVENT_GENERATOR)).newInstance();
	} catch (ClassNotFoundException e) {
		System.err.println("Invalid ClientEventGenerator class (Parameter: " + PAR_CLIENT_EVENT_GENERATOR + "). Terminating,");
		e.printStackTrace();
		return;
	} catch (InstantiationException e) {
		System.err.println("Exception while instantiaing ClientEventGererator class: '" + Configuration.getString(PAR_CLIENT_EVENT_GENERATOR) + "'" );
		e.printStackTrace();
		return;
	} catch (IllegalAccessException e) {
		System.err.println("Illega access while instantiaing ClientEventGererator class: '" + Configuration.getString(PAR_CLIENT_EVENT_GENERATOR) + "'" );
		e.printStackTrace();
		return;
	}
	clientManager.init(Configuration.getString(PAR_OPS_FILE));
	System.err.println("EDSimulator: running initializers");
	runInitializers();
	scheduleControls();

	//Load initial batch of client operations!!
	DCDataStoreSimulator.loadClientOperations();
	...
	DCCommonState.setPhase(DCCommonState.POST_SIMULATION);
...
private static void setEndtime() {
	...
	if( DCCommonState.getEndTime() < 0 ) {
		...
		System.err.println("New Simulation End Time: " + DCCommonState.getEndTime());
		if( heap.maxTime() < endtime )
			throw new IllegalParameterException(PAR_GRACE_PERIOD,
