package peersim.example.moodle.replicationProtocols;
...
import peersim.core.dcdatastore.replicationProtocol.HybridReplicationProtocol;
...
import peersim.example.moodle.replicationProtocols.events.CourseSimReadReply;
import peersim.example.moodle.replicationProtocols.events.ForumAddPostOperation;
import peersim.example.moodle.replicationProtocols.events.ForumUpdatePostOperation;
import peersim.example.moodle.replicationProtocols.events.MoodleReadOperation;
import peersim.example.moodle.replicationProtocols.events.MoodleWriteOperation;
import peersim.example.moodle.replicationProtocols.events.PageUpdateOperation;

public class BogusHybridReplicationProtocol extends HybridReplicationProtocol implements Cloneable {
	
	private List<PageUpdateOperation> operations;
	
	public BogusHybridReplicationProtocol (String name) {
		super(name);
		this.operations = new ArrayList<PageUpdateOperation>();
	}

	public BogusHybridReplicationProtocol clone() {
		BogusHybridReplicationProtocol hrp = null;
		hrp = (BogusHybridReplicationProtocol) super.clone();
		hrp.operations = new ArrayList<PageUpdateOperation>();
		return hrp;
	}
	

	//Copied from BogusPeriodicReplicationProtocol
	@Override
	...
	public void handleServerPropagationRequest(ServerNode node, int pid, OperationPropagationEvent event) {
		if (event instanceof SimpleOperationPropagationEvent){
			handleClientWriteRequest(node, pid, ((SimpleOperationPropagationEvent) event).getClientOperation());
		} else if (event instanceof MultipleOperationPropagationEvent) { 
